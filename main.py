...
# Importing art for ASCII banner (package #4)
from art import *
from functions import *
# Importing rich for further text formatting (package #2)
from rich import *
from rich.console import Console
# Importing colorama to colour input text (package #3)
...
print('Welcome to [bold cyan]Hi[/bold cyan]-[bold yellow]Lo[/bold yellow]!')
print('The aim is to create an equation that is closest to or equal to '
      '[bold cyan]Hi (20)[/bold cyan] or [bold yellow]Lo (1)[/bold yellow].')
print('Try to beat your high score! If you would like a break, '
      'type [pink]/quit[/pink] at the end of the round.')

if __name__ == "__main__":  # Separates code from pytest
        ...
        while True:
            ...
            if hilo_choice.lower() == 'hi':
                console.print(f'\nYou chose [bold cyan]{hilo_choice}[/bold cyan]! '
                              'Type the card ([dim bold]1-7[/dim bold]) and hit Enter to place it. '
                              'Type [purple]/reset[/purple] to restart your placed cards.', highlight=False)
            else:
            ...
            while len(played_cards) < 7:  # While the players list of played cards is less than 7 cards...
                try:  # Try this, and print the except at the bottom if it's not valid
                    ...
                    if 1 <= entered_card_number <= 7:  # If input is in between 1 and 7 (incl.), add that card to the players hand
                        ...
                        if chosen_card in played_cards:  # If the input is already in the played cards list, tell the player
                            ...
                            ('Your next card must be an operator')
                ...
                except ValueError:  # Checks for valid int
